...
/**
 * Shared magic-link verification UI + logic.
 *
 * Token/type arrive as props from one of two routes:
 *   - /auth/verify?token=XXX&type=magiclink   (legacy query form)
 *   - /auth/verify/magiclink/XXX              (path form — the one we email)
 *
 * Why the path form exists: email pipelines that re-decode quoted-printable
 * bodies treat `=` followed by two hex digits as an escape sequence. A hex
 * token after `token=` ALWAYS starts with two hex digits, so the `=` and the
 * first two token characters were being eaten in transit, breaking every
 * sign-in link. A path URL contains no `=` at all, so nothing can mangle it.
 *
 * Calls supabase.auth.verifyOtp({ token_hash, type }) — works regardless of
 * Supabase Site URL / Redirect URL config because we're not relying on
 * Supabase's own URL handling.
 *
...
export function VerifyClient({
  ...
  }, [router, token, typeParam, supabase])
